function showToast(message, type = 'success') {
    ...
    toast.className = `flex items-center w-full max-w-sm p-4 text-gray-500 bg-white rounded-lg shadow dark:text-gray-400 dark:bg-gray-800 border-l-4 ${isError ? 'border-red-500' : 'border-green-500'} transition-all duration-300 transform translate-y-full opacity-0`;
    
    const iconWrapper = document.createElement('div');
    iconWrapper.className = `inline-flex items-center justify-center flex-shrink-0 w-8 h-8 rounded-lg ${isError ? 'text-red-500 bg-red-100 dark:bg-red-800 dark:text-red-200' : 'text-green-500 bg-green-100 dark:bg-green-800 dark:text-green-200'}`;
    
    iconWrapper.innerHTML = isError 
